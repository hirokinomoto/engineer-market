...
職種名KW Group 1のGoogle Trends CSVを格納するrawテーブルです。
...
職種名KW Group 2のGoogle Trends CSVを格納するrawテーブルです。
...
また、Group 1とGroup 2には共通して `システムエンジニア` を含めています。  
これは、取得グループごとに0〜100で正規化されるGoogle Trendsのスケール差を補正するためです。

---

## 3. raw_google_trends_beginner_it_roles

### 役割

未経験IT職関連KWのGoogle Trends CSVを格納するrawテーブルです。
...
|---|---|---|
| month | DATE | 月初日 |
| it_beginner | INT64 | IT 未経験の検索関心指数 |
| engineer_beginner | INT64 | エンジニア 未経験の検索関心指数 |
| programmer_beginner | INT64 | プログラマー 未経験の検索関心指数 |

### 設計意図

未経験IT職関連KWは、職種名そのものではなく、未経験からIT職を目指す層の検索関心を見るための補助的なキーワードです。

そのため、職種名KWとは別のrawテーブルとして管理し、staging以降では `keyword_type = beginner_query` として区別します。

---

## 4. raw_estat_job_market_2012_2022
...
2012年は1月・2月が収録されていないため、年次比較の整合性を優先して2013年以降を対象にしています。

---

## 5. raw_estat_job_market_2023_2025
...
## 6. stg_google_trends_monthly

### 役割

Google Trendsの3つのrawテーブルを縦持ちに変換し、月次検索関心データとして統合します。
...
|---|---|---|
| month | DATE | 月初日 |
| keyword_type | STRING | role_name / beginner_query |
| keyword | STRING | 検索キーワード |
| trend_value | INT64 | Google Trendsの検索関心指数 |
| source_group | STRING | group_01 / group_02 / beginner_it_roles |
...
- 職種名KWと未経験IT職関連KWの切り分け

`keyword_type` を持たせることで、職種名KWと未経験IT職関連KWを同じstagingテーブルで扱いつつ、BI側では区別して表示できます。

---

## 7. stg_google_trends_adjusted_monthly
...
職種名KWはGroup 1とGroup 2に分かれているため、そのまま結合するとグループ間の比較基準がずれます。

そのため、共通して含まれる `システムエンジニア` をアンカーKWとして使い、Group 2をGroup 1基準に補正します。

補正後データでは、重複するGroup 2側のアンカーKWを除外します。  
これにより、`システムエンジニア` が2重に集計されることを防いでいます。

未経験IT職関連KWは職種名KWのアンカー補正対象ではありませんが、同じ形式で保持し、`keyword_type = beginner_query` として区別します。

---

## 8. stg_estat_job_market_monthly
...
また、2012-2022ファイルと2023-2025ファイルの重複期間を除外したうえで統合することで、2013〜2025年の一貫した月次データとして扱えます。
...
## 9. mart_engineer_market_monthly

### 役割

Google Trendsとe-Statの月次データを、データソース・カテゴリ種別・カテゴリ・指標別に比較可能な形でまとめます。
...
| category_type | STRING | role_name / beginner_query / occupation |
...
そのため、無理に横並びの1行にまとめるのではなく、`data_source`・`category_type`・`category`・`metric_name` を持たせた縦持ちの比較用テーブルにしています。

この形にすると、Looker Studioで以下を切り替えやすくなります。

- データソース
- 職種名KW / 未経験IT職関連KW / 職業分類
- カテゴリ
- 指標

---

## 10. mart_engineer_market_yearly

### 役割

2013〜2025年の長期推移を見やすくするため、月次martを年次集計したテーブルです。
...
2013〜2025年の長期推移を月次のまま見ると156か月分になり、傾向が見づらくなります。
...
このテーブル構成では、Google Trendsとe-Statを無理に同じ意味の指標として扱うのではなく、`data_source`・`category_type`・`category`・`metric_name`・`unit` を持たせて、BI上で切り替えながら確認できる形にしています。
