...
主な目的は、分析結果そのものを深掘りすることではなく、データエンジニア業務で必要となる以下の流れを再現することです。
...
データエンジニア職では、単にSQLを書くことだけでなく、取得元データを扱いやすい形に整え、分析基盤上で再利用しやすいテーブル設計を行い、BIツールで可視化できる状態までつなげる力が求められると考えています。

本プロジェクトでは、ITエンジニア市場を題材に、Google Trendsとe-Statの2種類の外部データを使用しました。

データ形式や粒度が異なる複数データソースを、BigQuery上で統一的に扱える形へ整備することで、実務に近いデータ処理フローを再現しています。

---

## 求人要件との対応

本プロジェクトでは、データエンジニア職の業務内容を想定し、以下の要素を実装しています。

| 想定される業務・要件 | 本プロジェクトで示している内容 |
|---|---|
...
| ナレッジ共有・ドキュメント化 | 実装サマリ、テーブル定義、BI構成メモをdocs配下に整理 |
